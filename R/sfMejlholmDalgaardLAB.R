#' @title Secondary growth rate model of Lactic Acid Bacteria (LAB) in RTE seafood
#'
#' @description
#' The function [sfMejlholmDalgaard()] estimates the growth rate of LAB in RTE seafood, based on the
#' predictive secondary model published by \insertCite{Mejlholm2013;textual}{qraLm}, and later validated in RTE foods including
#' seafood (\insertCite{Mejlholm2015}{qraLm}).
#'
...
#' @param CO2max (ppm) MIC for `CO_ 2`.
...
#' @param mumaxref (`1/h`) Maximum growth rate at the reference temperature \code{Tref}.
#'
#' @return mumax (`1/h`) Specific growth rate of LAB in RTE seafood at the 
#' given intrinsic and extrinsic characteristics (scalar, vector or matrix).
#'
#' @author Laurent Guillier
#'
#' @keywords secondary growth model lightly-preserved seafood psychrotrophic
#'
#' @references
#'
...
#'
#' @note
#' The parameters of the growth rate model are set by default to those fitted and 
#' validated by \insertCite{Mejlholm2015;textual}{qraLm}.
#' This model can also be found in the user friendly 
#' software FSSP: [](http://fssp.food.dtu.dk/Help/Listeria/Lm-LAB/lm-lab.htm).
#'
#' @examples
...
#'                       pH = 6.2, P = 5, laWph = 8000)
#'
sfMejlholmDalgaardLAB <- function(Temp,
                                  aw = NULL,
                                  NaCl = 0,
                                  pH = 7,
                                  P = 0,
  ...
  phi_acids <- (1 - ((1 - sqrt(LACu / micLACu)) * (1 - sqrt(DACu / micDACu)) * (1 - sqrt(AACu / micAACu)) * (1 - (BACu / micBACu)) * (1 - (CACu / micCACu)) * (1 - (SACu / micSACu))))^2
  ...
  mumax <- mumaxref * ((Temp - Tmin) / (Tref - Tmin))^2 * ((aw - awmin) / (1 - awmin)) * (1 - 10^(pHmin - pH)) * (1 - (LACu / micLACu)) *
    ((pheMax - P) / pheMax) * ((NITmax - NIT) / NITmax)^2 * ((CO2max - CO2) / CO2max) * (1 - sqrt(DACu / micDACu)) *
    (1 - sqrt(AACu / micAACu)) * (1 - (BACu / micBACu))^2 * (1 - (CACu / micCACu))^2 * (1 - (SACu / micSACu))^2 * xi
