...
#    h4("Risk computation"),
    fluidRow(
    column(8,
           h4("Risk summary statistics"), riskStatsUI("ca_risk_stats")),
    column(8,
           h4("Risk distribution (log10 risk)"), riskDistUI("ca_risk_dist")
    )
  )
  )
}
...
ca_Risk_server <- function(input, output, session, suffix, datStoredDices) {
  ...
  output[[id]] <- renderUI({ ca_Risk_ui(id) })

  prefix <- "rtecantaloupe-sidebar-inputs-"
  datRisk <- reactive({ generate_datRisk(input, prefix, datStoredDices) })

  riskStatsServer("ca_risk_stats",  data = datRisk)
  
  riskDistServer("ca_risk_dist",    data = datRisk)

  return(datRisk)
}

generate_datRisk <- function(input, prefix, datStoredDices) {
  df <- DRForModel(
    datStoredDices(),
