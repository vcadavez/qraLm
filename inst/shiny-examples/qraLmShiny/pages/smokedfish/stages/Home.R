sf_Home_ui <- function(id) {
  ...
  fluidRow(
  column(6,
         h4("Prevalence of contaminated lots"), prevLotsUI("sf_prev_lots_home"),
         h4("Between lots LM counts"), mcstatsLotsUI("sf_prod_lots_mcstats_home")
         #         h4("Between lots LM Counts distribution"), countsLotsDistUI("sf_prod_counts_lots_dist_home")
         ),
  column(6,
         h4("Prevalence of contaminated units"), prevUnitsUI("sf_prev_units_home"),
         h4("Within lots/Between units LM counts"), mcstatsUnitsUI("sf_prod_units_mcstats_home")
...
#  tagList(
    ...
    sliderInput(ns("temp_mode_h"),     "tempMode: Mode storage temperature (ºC)",
                value = 7, min = 3, max = 10, step=0.25),
    ...
    sliderInput(ns("time_mode_h"),     "timeMode: Mode storage time (h)",
                value = 50, min = 50, max = 100, step=1),
    sliderInput(ns("time_max_h"),     "timeMax: Maximum storage time (h)",
                value = 100, min = 100, max = 200, step=1),
