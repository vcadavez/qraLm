...
sf_Risk_server <- function(input, output, session, suffix, datPort) {
  ns <- NS(suffix)
  id <- ns("Risk")

  output[[id]] <- renderUI({ sf_Risk_ui(id) })

  prefix <- "smokedfish-sidebar-inputs-"
  datRisk <- reactive({ generate_datRisk(input, prefix, datPort) })

  riskStatsServer("sf_risk_stats",  data = datRisk)
  riskDistServer("sf_risk_dist",    data = datRisk)
